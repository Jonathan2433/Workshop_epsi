<template>
  <v-app class="random-background">
    ...
    >
      ...
      <v-list>
        <v-list-item v-for="[icon, text] in drawer_links" :key="icon" link>
          <v-list-item-icon>
            <v-icon>{{ icon }}</v-icon>
          </v-list-item-icon>
  ...
  methods: {
    ...
    getCentreFormation() {
      axios
        ...
        .then((response) => {
          this.centre_formations = response.data;
          if (this.centre_formations[0]) {
            console.log(this.centre_formations);
          } else {
            alert("Impossible de récupérer les centres de formation!");
          }
        })
        .catch((error) => {
          console.log(error);
